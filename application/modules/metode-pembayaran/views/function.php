<script type="text/javascript" charset="utf-8" async defer>
    ...
    function tampildata()
    {
      ...
      $("#myTable").DataTable({
        ...
        "serverSide": false,
        "ajax" :{
          url : "<?= site_url('metode-pembayaran/ajax') ?>",
          type: "POST",
              dataType: "json",
              data:{
                urutan:urutan
  ...
  $('body').on('click', '.add', function(event) {
    ...
    $.ajax({
      url: '<?= site_url('metode-pembayaran/modal_add') ?>',
      type: 'post',
      dataType: 'html',
      beforeSend: function(){   
  ...
  $('body').on('submit', '#simpan', function(event) {
      ...
      $.ajax({
        url: '<?= site_url('metode-pembayaran/simpan') ?>',
        type: 'post',
        dataType: 'html',
  ...
  $('body').on('click', '.edit', function(event) {
    ...
    /* Act on the event */
      ...
      $.ajax({
        url: '<?= site_url('metode-pembayaran/modal_edit') ?>',
        type: 'post',
        dataType: 'html',
        data: {id: id,},
        beforeSend: function(){   
  ...
  $('body').on('submit', '#perbarui', function(event) {
    ...
    $.ajax({
      url: '<?= site_url('metode-pembayaran/perbarui') ?>',
      type: 'post',
      dataType: 'html',
      data: new FormData(this),
      contentType: false,       
  ...
  $('body').on('click', '.delete', function(event) {
    ...
    $.ajax({
      url: '<?= base_url('metode-pembayaran/delete') ?>',
      type: 'post',
      dataType: 'html',
      beforeSend: function(){   
 ...
 $('body').on('click', '.hapus_data', function(event) {
   ...
   /* Act on the event */
    ...
    $.ajax({
        url: '<?= base_url('metode-pembayaran/hapus') ?>',
        type: 'post',
        dataType: 'html',
        beforeSend: function(){   
