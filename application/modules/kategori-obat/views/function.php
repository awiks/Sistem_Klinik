<script type="text/javascript" charset="utf-8" async defer>
    ...
    function tampildata()
    {
      ...
      $("#myTable").DataTable({
        ...
        "processing": true,
          "serverSide": false,
        "ajax" :{
          url : "<?= site_url('kategori-obat/ajax') ?>",
          type: "POST",
              dataType: "json",
              data:{
                urutan:urutan
  ...
  $('body').on('change', '.urutan', function(event) {
	...
	$('body').on('click', '.add', function(event) {
      ...
      $.ajax({
        url: '<?= site_url('kategori-obat/modal_add') ?>',
        type: 'post',
        dataType: 'html',
        beforeSend: function(){   
  ...
  $('body').on('submit', '#simpan', function(event) {
      ...
      $.ajax({
        url: '<?= site_url('kategori-obat/simpan') ?>',
        type: 'post',
        dataType: 'html',
    ...
    $('body').on('click', '.edit', function(event) {
	  ...
	  /* Act on the event */
	    ...
	    $.ajax({
	      url: '<?= site_url('kategori-obat/modal_edit') ?>',
	      type: 'post',
	      dataType: 'html',
        data: {id: id,},
	      beforeSend: function(){   
  ...
  $('body').on('submit', '#perbarui', function(event) {
    ...
    $.ajax({
      url: '<?= site_url('kategori-obat/perbarui') ?>',
      type: 'post',
      dataType: 'html',
      data: new FormData(this),
      contentType: false,       
  ...
  $('body').on('click', '.delete', function(event) {
    ...
    $.ajax({
      url: '<?= base_url('kategori-obat/delete') ?>',
      type: 'post',
      dataType: 'html',
      beforeSend: function(){   
 ...
 $('body').on('click', '.hapus_data', function(event) {
   ...
   /* Act on the event */
    ...
    $.ajax({
        url: '<?= base_url('kategori-obat/hapus') ?>',
        type: 'post',
        dataType: 'html',
        beforeSend: function(){   
