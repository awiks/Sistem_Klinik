<script type="text/javascript" charset="utf-8" async defer>
    ...
    function tampildata()
    {
      ...
      $("#myTable").DataTable({
        ...
        "serverSide": false,
        "ajax" :{
          url : "<?= site_url('dokter-jaga/ajax_dokter') ?>",
          type: "POST",
              dataType: "json",
              data:{
                urutan:urutan
  ...
  $('body').on('click', '.add', function(event) {
      ...
      $.ajax({
        url: '<?= site_url('dokter-jaga/modal_add_dokter') ?>',
        type: 'post',
        dataType: 'html',
        beforeSend: function(){   
  ...
  $('body').on('submit', '#simpan', function(event) {
      ...
      $.ajax({
        url: '<?= site_url('dokter-jaga/simpan_dokter') ?>',
        type: 'post',
        dataType: 'html',
  ...
  $('body').on('click', '.edit', function(event) {
	  ...
	  /* Act on the event */
	    ...
	    $.ajax({
	      url: '<?= site_url('dokter-jaga/modal_edit_dokter') ?>',
	      type: 'post',
	      dataType: 'html',
	      beforeSend: function(){   
  ...
  $('body').on('submit', '#perbarui', function(event) {
    ...
    $.ajax({
      url: '<?= site_url('dokter-jaga/perbarui_dokter') ?>',
      type: 'post',
      dataType: 'html',
      data: new FormData(this),
      contentType: false,       
  ...
  $('body').on('click', '.delete', function(event) {
    ...
    $.ajax({
      url: '<?= base_url('dokter-jaga/delete_dokter') ?>',
      type: 'post',
      dataType: 'html',
      beforeSend: function(){   
 ...
 $('body').on('click', '.hapus_data', function(event) {
   ...
   /* Act on the event */
    ...
    $.ajax({
        url: '<?= base_url('dokter-jaga/hapus_dokter') ?>',
        type: 'post',
        dataType: 'html',
        beforeSend: function(){   
